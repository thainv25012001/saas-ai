...
| Tool latency stacks | **Three** bounds enforce one declared budget, and they are deliberately different numbers — see the table in `docs/ARCHITECTURE.md` §7.3 for why. A tool's *database* work is bounded at `timeout_seconds` (default 10s) by `SET LOCAL statement_timeout`, so an overrun arrives as an ordinary statement error a savepoint recovers from rather than a cancelled connection that costs the whole turn. Its *non-database* work is bounded at `timeout_seconds + 5s` (`_NON_DB_GRACE_SECONDS`) by `asyncio.timeout`; the grace exists purely to guarantee the database bound always fires first, so a cancellation only ever lands when the tool is provably not inside a statement. The registry's outer bound is `timeout_seconds + 30s` (`_LOCK_WAIT_BUDGET_SECONDS`), a soft allowance for a call queued behind siblings sharing the turn's session, and it is never quoted back to the model as if it were the tool's own budget. Because a step dispatches all its calls together and all outer budgets expire together, a step's tool time is bounded near 40s **plus one tool budget regardless of how many calls it gathers** — so a 5-step turn is bounded nearer 250s than the 50s an earlier draft of this row claimed. Provider streaming time sits on top of that and is unbounded, which is pre-existing. A genuine whole-turn budget is Phase 5 (§8). Streaming means the user sees progress throughout. |
...
| **NUL sanitisation covers tool-call fields, not the assistant message's own `content`.** Model-derived `tool_call_id`, `tool_name`, tool arguments and tool results are stripped of NUL and truncated to their column widths (`_ToolCallRecord.build`). The answer text itself is written through unchanged, so a NUL in model prose would still fail the `messages` insert. | Contained, not prevented: that write now degrades to a logged, in-band `ChatError` rather than an uncaught exception (`send()`'s `except SQLAlchemyError`). Deliberate — silently rewriting the answer the user just read is a worse trade than silently truncating an audit identifier, and no provider has been observed emitting one. Revisit if one ever is. |
