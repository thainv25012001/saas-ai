...
customers. This repository is **Phase 1 only**: authentication, organizations, agents,
and prompts, wired together end to end with no LLM call yet. Later phases (chat,
retrieval, tool-calling, evaluation, MCP, billing) build on this foundation; see
...
cp .env.example .env
docker compose up
make seed          # see "Without make", below, if you don't have it
```
...
applies every Alembic migration, and then starts the API and the dashboard.
...
| `make test` | Runs the backend test suite (118 tests: unit + integration, incl. tenant isolation and RLS). | `cd apps/api && uv run pytest -v` |
...
| `make seed` | Creates the demo organization/user/agent/prompt (idempotent). **Refuses to run unless `ENVIRONMENT=local`** — see [Before you deploy](#before-you-deploy). | `cd apps/api && uv run python -m app.db.seed` |
...
| 2 — Basic LLM chat | Next.js → chat API → LLM provider → streaming response. OpenAI first, then an Anthropic adapter behind the same interface. | Not started |
